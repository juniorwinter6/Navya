...
module.exports = {
    ...
    execute: async (sock, m, args, { isOwner }) => {
        ...
        if (rawTarget.includes("@lid") || targetClean.length >= 14) {
            if (from.endsWith("@g.us")) {
                try {
                    ...
                    console.error("[ADDSUDO] Group metadata lookup error:", e);
                }
            }
        }
        ...
        if (!cleanNum || cleanNum.length < 7) {
            await sock.sendMessage(from, {
                text: "❌ *Could not resolve a valid phone number. Try typing the full number manually (e.g. .addsudo 2348123456789).* "
            }, { quoted: m });
            return;
        }

        // 4. Save to lib/sudo.json
        ...
        if (sudoList.includes(cleanNum)) {
            await sock.sendMessage(from, {
                text: `⚠️ *@${cleanNum}* is already in the SUDO list!`,
        ...
        await sock.sendMessage(from, {
            text: `✅ *@${cleanNum}* has been added to SUDO!`,
            mentions: [`${cleanNum}@s.whatsapp.net`]
        }, { quoted: m });
    }
};